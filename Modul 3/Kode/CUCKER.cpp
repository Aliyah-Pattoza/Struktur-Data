...
private:
    ...
    void _inorder(AVLNode *node, AVLNode *par)
    {
        if (node)
        {
...
int main()
{
    ...
    while (t--)
    {
        int x;
        cin >> x;
        AVLNode *par = bst.find(x)->parent;
        int vPar = (par != NULL ? par->data : 0);
        AVLNode *grandPar = par->parent;
        int vGrandPar = (grandPar != NULL ? grandPar->data : 0);
        int siblings = 0;
        if (grandPar != NULL)
        {
            if (grandPar->left != NULL && grandPar->left->data != vPar)
                siblings = grandPar->left->data;
            if (grandPar->right != NULL && grandPar->right->data != vPar)
                siblings = grandPar->right->data;
        }
        cout << abs(siblings - vPar) << endl;
    }
    return 0;
}
